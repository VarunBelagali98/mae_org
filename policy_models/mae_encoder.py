...
class MAEViTEncoder(nn.Module):
    ...
    def forward_features(self, x, pos_embed=None, block_index=None):
        ...
        if self.use_pe:
            if x.size(1) != pos_embed.size(1):
                ...
                #print("cls_pe shape", cls_pe.shape, "pos_emd shape", pos_embed.shape)
    ...
    def forward(self, x, pos_embed=None, block_index=None):
        if self.return_patchembed:
            x, patch_embed = self.forward_features(x, pos_embed, block_index)
        else:
            x = self.forward_features(x, pos_embed, block_index)
        x = self.head(x)
        if self.return_patchembed:
